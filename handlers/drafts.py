...
from keyboards.main_kb import cancel_kb, drafts_menu
...
async def show_drafts(message: Message):
    ...
    for draft in storage.drafts:
        ...
        text += f"   📍 Получатели: {len(draft['target_ids'])}\n"
        text += f"   📱 Аккаунты: {len(draft['accounts'])}\n\n"
...
async def configure_draft_start(message: Message, state: FSMContext):
    ...
    text = "Выберите черновик (номер):\n\n"
    for draft in storage.drafts:
        text += f"{draft['id']}. {draft['text'][:30]}...\n"
...
async def process_draft_send(message: Message, state: FSMContext):
    ...
    if not draft["target_ids"]:
        await state.clear()
        await message.answer("❌ Сначала настройте получателей!", reply_markup=drafts_menu())
        return
    ...
    for target_id in draft["target_ids"]:
        if target_id in storage.targets:
            ...
            assigned = draft["accounts"].copy() if draft["accounts"] else target_data.get("assigned_accounts", []).copy()
    ...
    await state.clear()
    await message.answer(f"✅ Черновик отправлен! Всего: {total_sent}", reply_markup=drafts_menu())
...
async def process_draft_deletion(message: Message, state: FSMContext):
    draft_id = int(message.text)
    draft = next((d for d in storage.drafts if d["id"] == draft_id), None)
    ...
    if draft:
        ...
        storage.save_drafts()
        await state.clear()
        await message.answer(f"✅ Черновик #{draft_id} удален!", reply_markup=drafts_menu())
    else:
        await message.answer("❌ Черновик не найден! Попробуйте снова:")